...
int main(){
    std::map<std::string,imu> ImuMap=imu::getImuMapFromDataFile(imutestutils::getTestDataFile("apdm_v5_multiple_sensors.h5"));
    imu::printLabelsInFile(imutestutils::getTestDataFile("apdm_v5_multiple_sensors.h5"));
    imu imu1=ImuMap["Sacrum"];
    imu cutImu=imu1.cutImuByIdx(10, 20);
    // write chopped imu
    std::string imuFileOut=imutestutils::getProjectRootDir()+"/test/output/testfile.h5";
    ...
    std::string imuMapFileOut=imutestutils::getProjectRootDir()+"/test/output/testfilemap.h5";
